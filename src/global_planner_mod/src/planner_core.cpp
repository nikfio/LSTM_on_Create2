...
#include <global_planner_mod/dijkstra.h>
#include <global_planner_mod/astar_mod.h>
#include <global_planner_mod/grid_path.h>
#include <global_planner_mod/gradient_path.h>
#include <global_planner_mod/quadratic_calculator.h>

//register this planner as a BaseGlobalPlanner plugin
PLUGINLIB_EXPORT_CLASS(global_planner_mod::GlobalPlanner, nav_core::BaseGlobalPlanner)
...
void GlobalPlanner::initialize(std::string name, costmap_2d::Costmap2D* costmap, std::string frame_id) {
    if (!initialized_) {
        ...
        if (use_dijkstra)
        {
            ...
            planner_ = new AStarExpansion(p_calc_, cx, cy, M_PI_4);
        ...
        dsrv_ = new dynamic_reconfigure::Server<global_planner_mod::GlobalPlannerModConfig>(ros::NodeHandle("~/" + name));
        dynamic_reconfigure::Server<global_planner_mod::GlobalPlannerModConfig>::CallbackType cb = boost::bind(
...
void GlobalPlanner::reconfigureCB(global_planner_mod::GlobalPlannerModConfig& config, uint32_t level) {
...
bool GlobalPlanner::makePlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal,
    ...
    if (!costmap_->worldToMap(wx, wy, goal_x_i, goal_y_i)) {
        ROS_WARN_THROTTLE(1.0,
                "The goal sent to the global planner is off the global costmap. Planning will always fail to this goal. [LEGACY]\n"
		"MODIFICATION: manage a goal off the global map, get closer then update map and try with another iteration.");
        //return false;
    }
    if(old_navfn_behavior_){
        goal_x = goal_x_i;
...
//	std::vector<std::pair<float,float>> path_found;
//	int nodes_exp = 0;

//	calculatePath(costmap_, start_x, start_y, goal_x, goal_y, nx * ny * 2, potential_array_, path_found, nodes_exp, true);
//	
//	    ros::Time plan_time = ros::Time::now();
//	for (int i = 0; i < path_found.size(); i++) {
//        std::pair<float, float> point = path_found[i];
